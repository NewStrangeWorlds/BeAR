...
observation = np.loadtxt(retrieval_folder+"gj570d_spex.dat", skiprows=11)
# obs_data = list([observation[:,1]])
# obs_error = list([observation[:,2]])

obs_data = observation[:,1]
obs_error = observation[:,2]
log_like_weight = observation[:,4]
...
def loglike(cube, ndim, nparams):
  ...
  error_inflation = 10**physical_parameters[-1]
  ...
  for i in range(obs_data.size):
    error_square = obs_error[i]**2 + error_inflation
    log_like += (- 0.5 * np.log(error_square* 2.0 * np.pi)
                 - 0.5 * obs_delta[i]**2 / error_square) * log_like_weight[i]
