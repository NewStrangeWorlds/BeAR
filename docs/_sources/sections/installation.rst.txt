...
standard. Furthermore, it needs the ``CUDA`` compiler and framework from NVIDIA. 
Additional libraries and codes (``FastChem``, ``MultiNest``, ``CDisort``, ``Boost``) 
will be downloaded from their respective repositories and be compliled as well.
...
the installation of ``BeAR`` have addiational requirements. This includes, in particular,
