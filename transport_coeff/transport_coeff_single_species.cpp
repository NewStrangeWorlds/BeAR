...
void TransportCoefficientsSingleSpecies::init(std::vector<size_t>& sampling_index_list)
{
  ...
  for (unsigned int i=0; i<cross_section_data.size(); i++)
    sampled_cross_sections[i].init(cross_section_data[i].temperature,cross_section_data[i].pressure);
}
...
void TransportCoefficientsSingleSpecies::readFileList(const std::string file_path)
{
  ...
  if (file.fail())
  {
    std::cout << "no absorption cross-sections for species " << species_name << " found! Ignoring from here on." << std::endl;
...
double TransportCoefficientsSingleSpecies::linearInterpolation(double x1, double x2, double y1, double y2, double x)
{
...
void TransportCoefficientsSingleSpecies::checkDataAvailability(double local_pressure, double local_temperature)
{
  unsigned int lower_index1 = 0; 
  unsigned int higher_index1 = 0; 
  unsigned int lower_index2 = 0;
                                                                   ...
                                                                   std::vector<double>& absorption_coeff, std::vector<double>& scattering_coeff)
{
  ...
  if (species_name == "K" || species_name == "Na") gas_pressure = number_densities[_H2] * CONST_K * temperature / 1e6;
...
double TransportCoefficientsSingleSpecies::generalRayleighCrossSection(double reference_density, double refractive_index, double king_correction_factor,
                                                          double wavenumber)
{
  return 24. * pow(CONST_PI, 3) * pow(wavenumber, 4) / (reference_density*reference_density)
...
void CrossSectionFile::loadFile()
{
  ...
  if (file.fail()) std::cout << "cross section file " << filename << " not found! This shouldn't happen... :(((( \n";
