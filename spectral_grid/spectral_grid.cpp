...
SpectralGrid::SpectralGrid(GlobalConfig* global_config)
{
  ...
  std::cout << config->wavenumberFilePath() << "\n";
...
bool SpectralGrid::loadWavenumberList()
{
  std::string file_name = config->wavenumberFilePath();
  ...
  if (file.fail())
  {
    std::cout << "Could not open wavenumber file: " << file_name << "\n";
    return false;
  }
