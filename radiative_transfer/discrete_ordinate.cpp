...
extern "C" {
  #include "../cdisort/cdisort_macros.h"
...
DiscreteOrdinates::DiscreteOrdinates(Retrieval* retrieval_ptr, const size_t nb_streams, const size_t nb_grid_points)
{
                                     ...
                                     std::vector<double>& spectrum)
{
  ...
  for (size_t i=0; i<spectrum.size(); ++i)
    spectrum[i] = calcSpectrum(absorption_coeff[i], scattering_coeff[i], vertical_grid, i);
...
double DiscreteOrdinates::calcSpectrum(const std::vector<double> absorption_coeff, const std::vector<double> scattering_coeff,
                                       const std::vector<double>& vertical_grid, const size_t nu_index)
{
  ...
  double wavenumber = retrieval->spectral_grid.wavenumber_list[nu_index];
