...
void FlatLine::setPriors(Priors* priors)
{
  ...
  if (prior_type.size() != nb_total_param())
  {
    ...
    throw InvalidInput(std::string ("FlatLine::setPriors"), error_message);
  }
...
void FlatLine::readPriorConfigFile(
  ...
  std::vector<std::vector<double>>& prior_parameter)
{
  ...
  if (file.fail())  
    throw FileNotFound(std::string ("FlatLine::readPriorConfigFile"), file_path);
