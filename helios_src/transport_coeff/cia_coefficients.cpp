...
void CIACoefficients::readGeneralCIAFile(const std::string& file_name, const std::vector<double>& wavenumber_grid)
{
  ...
  if (file.fail())
    throw ExceptionFileNotFound(std::string ("CIACoefficients::readGeneralCIAFile"), file_name);
...
{
  const unsigned int nb_data_points = wavenumber_data.size();
  std::vector<double> result(wavenumber_grid.size(), 0.0);



  //lambda function for linear interpolation
  auto interpolation = [](double x1, double x2, double y1, double y2, double x)
  ...
  {
    const double wavenumber = wavenumber_grid[i];

    if (wavenumber > wavenumber_data.back() || wavenumber < wavenumber_data.front())
    {
      result[i] = 1e-300; continue;
    }


    if (wavenumber == wavenumber_data.back())
    {
      result[i] = data.back(); continue;
    }


    for (size_t j=1; j<nb_data_points; ++j)
    {
      if (wavenumber == wavenumber_data[j])
      {
        result[i] = data[j]; break;
      }
      ...
      {
        if (interpolate_log)
          result[i] = std::pow(10, interpolation(wavenumber_data[j-1], wavenumber_data[j], std::log10(data[j-1]), std::log10(data[j]), wavenumber));
        else
          result[i] = interpolation(wavenumber_data[j-1], wavenumber_data[j], data[j-1], data[j], wavenumber);
