...
class BasicPrior{
  public:
    virtual double priorParameterValue(const double& hypercube_parameter) = 0;  //returns the value of the prior for a given parameter
    virtual ~BasicPrior() {}                                                    //constructor
    std::string priorName() {return prior_name;}                                //returns the prior type
    std::string parameterName() {return parameter_name;}                        //returns the parameter name     
...
class UniformPrior : public BasicPrior {
  public:
    UniformPrior (const std::string& parameter,
                  const double& upper_bnd,
                  const double& lower_bnd)
                  {parameter_name = parameter; upper_bound = upper_bnd; lower_bound = lower_bnd; prior_name = "uniform prior";}
    virtual ~UniformPrior() {}
    virtual double priorParameterValue(const double& hypercube_value) {return lower_bound + hypercube_value * (upper_bound - lower_bound);}
  private:
...
class LogUniformPrior : public BasicPrior {
  public:
    LogUniformPrior (const std::string& parameter,
                     const double& upper_bnd,
                     const double& lower_bnd)
                     {parameter_name = parameter; log_upper_bound = std::log10(upper_bnd); log_lower_bound = std::log10(lower_bnd); prior_name = "log-uniform prior";}
    virtual ~LogUniformPrior() {}
    virtual double priorParameterValue(const double& hypercube_value) {return std::pow(10.0, log_lower_bound + hypercube_value * (log_upper_bound - log_lower_bound));}
  private:
...
class GaussianPrior : public BasicPrior {
  public:
    GaussianPrior (const std::string& parameter,
                   const double& mean,
                   const double& standard_deviation)
                   {parameter_name = parameter; mu = mean; sigma = standard_deviation; prior_name = "Gaussian prior";}
    virtual ~GaussianPrior() {}
    virtual double priorParameterValue(const double& hypercube_value) {return mu + sigma * std::sqrt(2.0) * boost::math::erf_inv(2.*hypercube_value - 1.);}
  private:
    double mu = 0;
    double sigma = 0;
};
